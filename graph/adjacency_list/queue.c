...
Init_Queue ( )
{
    ...
    /*  create list header */ 
    new_queue->queue_list	= malloc ( sizeof(struct queue_list_node) );
...
Enqueue ( Queue my_queue , int index )
{
    ...
    new_node	= malloc ( sizeof(struct queue_list_node) );
    if ( new_node==NULL ) {
        fprintf ( stderr, "\ndynamic memory allocation failed\n" );
        exit (EXIT_FAILURE);
    }
...
{
    Queue_list_node dequeue_node=NULL;
    if(my_queue == NULL)
    {
        fprintf(stderr, "\n the queue have not init \n ");
        return NULL;
